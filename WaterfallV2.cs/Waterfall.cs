...
namespace WindowsGSM.Plugins
{
    public class Waterfall
    {
        ...
        {
            name = "WindowsGSM.WaterfallV2", // WindowsGSM.XXXX
            author = "VisualYak",
            description = "Adds Waterfall proxy v2 support for Minecraft: Paper servers.",
            version = "2.0",
            url = "https://github.com/VisualYak/WindowsGSM.WaterfallV2", // Github repository link (Best practice)
        ...
        public async Task<Process> Install()
        {
            ...
            try
			{
				...
				{
					await webClient.DownloadFileTaskAsync("https://api.papermc.io/v2/projects/waterfall/versions/1.20/builds/562/downloads/waterfall-1.20-562.jar", ServerPath.GetServersServerFiles(_serverData.ServerID, StartPath));
				}
			}
            catch (Exception e)
            {
		...
		{
			// Get latest version and build at https://api.papermc.io/v2/projects/waterfall
			try
			{
				using (var webClient = new WebClient())
				{
					// Get the latest version
					var versionJson = await webClient.DownloadStringTaskAsync("https://api.papermc.io/v2/projects/waterfall");
					var latestVersion = JObject.Parse(versionJson)["versions"].Last.ToString();

					// Get the latest build for the latest version
					var buildJson = await webClient.DownloadStringTaskAsync($"https://api.papermc.io/v2/projects/waterfall/versions/{latestVersion}");
					var latestBuild = JObject.Parse(buildJson)["builds"].Last.ToString();
        ...
        public static async Task<bool> DownloadGameServerConfig(string filePath, string gameFullName)
        {
            ...
            try
            {
                ...
                {
                    await webClient.DownloadFileTaskAsync($"https://raw.githubusercontent.com/VisualYak/WindowsGSM.WaterfallV2/Waterfall.cs/config.yml", filePath);
                }
            } catch (Exception e)
            {
